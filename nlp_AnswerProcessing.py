...
def getScoreDict(answerdict, standardAnswer):
    ...
    print("  + 正在计算TF-IDF矩阵.....")
    weight = nlp_tfidf.getTfidf(seganswersdoc)
    weight = nlp_tfidf.getTopK(20, weight)

    fullscore = nlp_tfidf.getCosine(weight[0], weight[0])
    cnt = 1
    numberlist = [0] * 7
    averagescore= [0] * 7
    print(" + 开始统计成绩.....")
...
def readAnswersdoc():
    ...
    with open(web_options.getCachePath('answers'), 'r') as file:
